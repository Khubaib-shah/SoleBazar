...
export default function SettingsPage() {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-10">
            {/* Header / Tabs */}
            <div className="rounded-[24px] md:rounded-[40px] shadow-sm border border-[#E8DCC8] p-4 md:p-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between sticky top-[64px] z-10 backdrop-blur-xl bg-white/90">
                <div className="flex-1 w-full md:w-auto">
                    {/* Mobile Tabs Custom Dropdown */}
                    <div className="md:hidden w-full relative">
                        <button
                            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                            className="w-full flex items-center justify-between bg-[#FAFAF7] border-2 border-[#E8DCC8] rounded-2xl px-6 py-4 font-black text-[10px] uppercase tracking-widest text-[#2B2B2B] focus:outline-none focus:border-[#7C8C5C] shadow-sm"
                        >
                            <span className="flex items-center gap-3">
                                {(() => {
                                    const TabIcon = tabs.find(t => t.id === activeTab)?.icon;
                                    return TabIcon ? <TabIcon className="w-4 h-4 text-[#7C8C5C]" /> : null;
                                })()}
                                {tabs.find(t => t.id === activeTab)?.label}
                            </span>
                            <ChevronDown className={`w-4 h-4 text-[#7C8C5C] transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>

                        {isDropdownOpen && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-[#E8DCC8] rounded-[24px] shadow-2xl overflow-hidden z-[100] animate-in fade-in slide-in-from-top-2 duration-300">
                                {tabs.map((tab) => (
                                    <button
                                        key={tab.id}
                                        onClick={() => {
                                        ...
                                        className={`w-full flex items-center gap-4 px-6 py-4 text-left font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === tab.id
                                            ? "bg-[#2B2B2B] text-white"
                                            : "bg-white text-[#555] hover:bg-[#FAFAF7] hover:text-[#7C8C5C]"
                                            }`}
                                    >
                                        <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? "text-[#7C8C5C]" : ""}`} />
                                        {tab.label}
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className="hidden md:flex gap-2">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`flex items-center gap-3 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all duration-300 ${activeTab === tab.id
                ...
                <div className="flex gap-4 justify-end">
                    ...
                    <button
                        onClick={handleSave}
                        disabled={isSaving}
                        className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 md:px-8 py-3.5 bg-[#7C8C5C] hover:bg-[#5C6C3C] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-50 min-w-0"
                    >
                ...
                <div className="lg:col-span-2 space-y-8">
                    {activeTab === 'general' && (
                        <div className="bg-white rounded-[48px] shadow-sm border border-[#E8DCC8] overflow-hidden">
                            <div className="p-5 md:px-10 border-b border-[#E8DCC8]">
