...
export default function OrderModal({ product, selectedSize, selectedColor }: { product: Product, selectedSize?: string, selectedColor?: string }) {
        ...
        <>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                    <div className="absolute inset-0 bg-[#2B2B2B]/80 backdrop-blur-xl" onClick={() => setIsOpen(false)}></div>
                    ...
                    <div className="bg-white w-full max-w-xl rounded-[40px] shadow-2xl relative z-10 overflow-hidden flex flex-col md:flex-row">
                        ...
                        <div className="hidden md:flex md:w-48 bg-[#FAFAF7] border-r border-[#E8DCC8] p-8 flex-col justify-between">
                            ...
                            <div className="space-y-6">
                                <div className="flex flex-col gap-1">
                                    <p className="text-[10px] font-black uppercase text-[#999] tracking-widest">Selected Item</p>
                                    <p className="font-bold text-sm text-[#2B2B2B] leading-tight">{product.name}</p>
                                </div>
                                <div className="flex flex-col gap-1">
                                    <p className="text-[10px] font-black uppercase text-[#999] tracking-widest">Price</p>
                                    <p className="font-black text-[#7C8C5C]">PKR {product.price.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 p-8 sm:p-12">
                            ...
                            <div className="mb-10">
                                <h2 className="text-3xl font-black text-[#2B2B2B] mb-2">Order Details</h2>
                                <p className="text-[10px] font-black uppercase tracking-widest text-[#7C8C5C]">Secure Checkout | Cash on Delivery</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-6 py-4 bg-[#FAFAF7] border-2 border-[#E8DCC8] rounded-3xl focus:outline-none focus:border-[#7C8C5C] font-bold text-sm transition-all"
                                        placeholder="e.g. Ali Ahmed"
                                    />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-[#555] ml-4">Phone Number</label>
                                    <input
                                        type="tel"
                                        required
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-[#555] ml-4">Shipping Address</label>
                                    <textarea
                                        required
                                        value={formData.address}
                                        onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                                        className="w-full px-6 py-4 bg-[#FAFAF7] border-2 border-[#E8DCC8] rounded-3xl focus:outline-none focus:border-[#7C8C5C] font-bold text-sm transition-all resize-none"
                                        rows={3}
                                ...
                                <div className="pt-4 space-y-4">
                                    ...
                                    <div className="flex items-center justify-center gap-6 pt-4 border-t border-[#E8DCC8]">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[8px] font-black uppercase tracking-widest text-[#999]">Verified Product</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Truck className="w-4 h-4 text-[#7C8C5C]" />
                                            <span className="text-[8px] font-black uppercase tracking-widest text-[#999]">Pay on Delivery</span>
                                        </div>
                                    </div>
                                </div>
                            </form>
